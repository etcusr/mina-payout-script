...
if min_height == 0:
    try:
        r = requests.get(f'https://api.minastakes.com/epoch/{staking_epoch}?fork=1')
        min_height = r.json()["minBlockHeight"]

    except Exception as min_height_err:
        print(f'{min_height_err}')
        exit()
